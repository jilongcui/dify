...
const AppCover: FC<AppCoverProps> = ({
  size = 'medium',
  rounded = false,
  iconType,
  icon,
  ...
  return <div
    className={classNames(AppCoverVariants({ size, rounded }), 'w-full h-full overflow-hidden', className)}
    style={{ background: isValidImageIcon ? undefined : (background || '#FFEAD5') }}
    onClick={onClick}
  >
    {isValidImageIcon
      // eslint-disable-next-line @next/next/no-img-element
      ? <img src={imageUrl} className="object-cover w-full h-full" alt="app icon" />
